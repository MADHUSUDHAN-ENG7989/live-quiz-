...
if (process.env.GROQ_API_KEY) {
    ...
    console.warn("⚠️ GROQ_API_KEY missing. AI features will be disabled.");
    groq = {
        chat: {
            completions: {
                create: async () => { throw new Error("Groq API Key missing"); }
            }
        }
    };
}
...
import seedLocalData from './seed_local_data.js';

console.log("DEBUG: Starting DB connection...");
connectDB().then(async () => {
    console.log("DEBUG: DB Connected. Seeding...");
    try {
        await seedLocalData();
        console.log("DEBUG: Seeding complete. Restoring state...");
        await restoreActiveQuizState();
        console.log("DEBUG: State restored.");
    } catch (e) {
        console.error("DEBUG: Startup Error:", e);
    }
}).catch(err => console.error("DEBUG: Fatal DB connection error:", err));
